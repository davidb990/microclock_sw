...
int run_startup(void) {
    ...
    dl2416t_display_word(&display, "PWR-");
    sleep_ms(1000);
    dl2416t_display_word(&display, "-ON!");
    sleep_ms(1000);
    return ADJUST_HRS;
}
...
int run_set_menu() {
    ...
    while (1) {
        ...
        switch (menu_sel) {
            case MENU_TIME  : 
                dl2416t_display_word(&display, "TIME");
                sleep_ms(15);
                break;
            case MENU_ALARM : 
                dl2416t_display_word(&display, "ALRM");
                sleep_ms(15);
                break;
            case MENU_ALARM_OFF   : 
                dl2416t_display_word(&display, "AOFF");
                sleep_ms(15);
                break;
...
int run_alarm_active(void) {
    ...
    while(1) {
        ...
        dl2416t_blank(&display, powman_hw->read_time_lower & 0b10000000 ? true : false);
...
int run_disable_alarm(void) {
    ...
    gpio_put(BUZ, false);
    sleep_ms(400);
    dl2416t_display_word(&display, "OFF ");
    sleep_ms(600);
    gpio_put(BUZ, true);
    sleep_ms(600);
    gpio_put(BUZ, false);
